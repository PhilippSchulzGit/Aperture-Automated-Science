    ...
    public void runRecognitionCMU()
    {
        ...
        Thread voiceRecognitionManagerThread = new Thread(() ->
        {
            ...
            while(voiceRecognitionCMU.getIsRunning() && isAlive)
            {
                ...
                {
                    // create action to print action to terminal
                    Action terminalPrint = new Action(this.idManager.getComponentIDByName(TERMINAL_MANAGER_NAME),this.ownID,
                            WAIT_FOR_EXECUTION_FALSE,ACTION_TERMINAL_MANAGER_PRINT+recognitionResult,ERROR_COUNT_NEW);
                    // add action to ActionQueue
                    this.actionQueue.addNewActionToActionQueue(terminalPrint);
                }
