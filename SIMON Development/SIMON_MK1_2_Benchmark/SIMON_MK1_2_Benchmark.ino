 ...
 * SIMON MK1.2 is used here.
 * This version adjusts the changed signaling from MK1.1 to be more efficient.
 * This is achieved by utilizing both states of the clock signal, effectively
...
void loadPayload(char* text) {
  ...
  if(charCounter > characterLimit) {					        // handle larger String than buffer, cut off rest
    charCounter = characterLimit;
  }
...
void readData(bool awaitResponse) {                   
  ...
  if(awaitResponse) {                                 // response handling is different than rest
    ...
    while(signalState == newSignalState) {            // lock until the next bit comes in or exit condition is met
